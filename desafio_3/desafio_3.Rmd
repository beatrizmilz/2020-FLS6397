...
```{r}
anti_join(consulta_cand_2016_RR, votacao_candidato_munzona_2016_RR, by = "SQ_CANDIDATO") 

# anti_join(consulta_cand_2016_RR, votacao_candidato_munzona_2016_RR, by = "SQ_CANDIDATO") %>% 
#   filter(DS_SITUACAO_CANDIDATURA == "INAPTO" | DS_DETALHE_SITUACAO_CAND %in% c("RENÚNCIA|INDEFERIDO"))
```
...
```{r}
left_join(candidatos, resultados_votos_totais, by = "SQ_CANDIDATO")
```
...
eleicoes_vereador %>%
  ...
  tally() %>%
  ggplot() +
  geom_col(aes(x = reorder(NM_MUNICIPIO , n), y = n)) +
  coord_flip() + 
  theme_bw() +
  labs(title = "Número de candidatos(as) para o cargo de vereador(a) \n por município", subtitle = "Na eleição de 2016 para o estado de RR",
       x = "Município", y = "Número de candidatos(as)"
          )
```
...
eleicoes_vereador %>% 
  ...
  ggplot() +
  geom_col(aes(x = DS_GENERO, y = n)) +
  theme_light() +
  labs(x = "Gênero", y = "Número de candidatos(as)", title = "Número de candidatos(as) para o cargo de vereador(a) \n por gênero", subtitle = "Na eleição de 2016 para o estado de RR")
```
...
eleicoes_vereador %>% 
  ...
  ggplot() +
  geom_col(aes(x = reorder(NM_MUNICIPIO , n), y = n, fill = DS_GENERO), position = "fill") +
  coord_flip() + 
  theme_light() +
  labs(x = "Município", 
       y = "Porcentagem (%)", 
...
```{r}
eleicoes_vereador <- eleicoes_vereador %>% mutate(binario_eleito = case_when(
...
genero_porcentagem <- eleicoes_vereador %>% group_by(NM_MUNICIPIO, DS_GENERO) %>%
  summarise(
    total_eleito = sum(binario_eleito == TRUE),
    total_nao_eleito = sum(binario_eleito == FALSE)
  ) %>% 
  mutate(total = total_eleito + total_nao_eleito,
...
genero_porcentagem  %>% 
  ggplot() +
  geom_col(aes(x = reorder(NM_MUNICIPIO , porcentagem_eleito), y = porcentagem_eleito, fill = DS_GENERO), position = "dodge") +
  coord_flip() + 
  theme_light() +
  labs(x = "Município", 
